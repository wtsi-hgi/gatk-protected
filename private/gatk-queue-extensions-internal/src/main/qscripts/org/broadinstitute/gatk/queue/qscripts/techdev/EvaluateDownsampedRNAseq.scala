...
class EvaluateDownsampedRNAseq extends QScript {
  qscript =>
  ...
  trait UNIVERSAL_GATK_ARGS extends CommandLineGATK {
    memoryLimit = 2
    reference_sequence = new File(qscript.ref)
  }
  ...
  def script = {
    ...
    for (index <- 0 until bams.size) {
      ...
      val bcd = new BaseCoverageDistribution with UNIVERSAL_GATK_ARGS
      bcd.input_file :+= bam
      bcd.intervals = qscript.intervalFiles
      bcd.intervalsString = qscript.intervalString
      bcd.out = swapExt(bam, ".bam", ".cov.grp")

      val cb = new CountBases with UNIVERSAL_GATK_ARGS
      cb.input_file :+= bam
      cb.jobOutputFile = swapExt(bam, ".bam", ".cb.out").toString

      add(bcd, cb)

      val vcfs = make_calls(bam)
      evaluate_precision(bam, vcfs)
   ...
   */
  def make_calls(bam: File) : (File, File) = {
    val ug_out = if (no_ug) {
      null
    } else {
      val ug_output = swapExt(bam, ".bam", ".ug.vcf")
      val ug = new ug_rnaMode(bam,ug_output)
      add(ug)
    ...
    val hc_out = if (no_hc) {
      ...
      val hc = new hc_rnaMode(bam, swapExt(bam, ".bam", ".hc.vcf"))
      add(hc)
  ...
  def evaluate_precision(bam: File, vcfs: (File, File)) {
    // unwind the tuple into better names
    val ugVCF = vcfs._1
    val hcVCF = vcfs._2
    val kb = new AssessNA12878 with UNIVERSAL_GATK_ARGS
    if (!no_ug){
      kb.V :+= new TaggedFile(ugVCF, "UG")
      val ug_assess = new assess(ugVCF, bam)
      add(ug_assess)
    }
    if (!no_hc){
      kb.V :+= new TaggedFile(hcVCF, "HC")
      val hc_assess = new assess(hcVCF, bam)
      add(hc_assess)
    }
    kb.out = swapExt(bam, ".bam", ".grp")
    kb.BAM = bam
    kb.detailed = true
    kb.badSites = swapExt(bam, ".bam", ".badsites.vcf")
    kb.intervalsString :+= "20:10000000-24000000"
    kb.intervals :+= new File("/seq/references/HybSelOligos/whole_exome_illumina_coding_v1/whole_exome_illumina_coding_v1.Homo_sapiens_assembly19.targets.interval_list")
    kb.excludeIntervals :+= new File("/humgen/gsa-hpprojects/NA12878Collection/knowledgeBase/complexRegions.doNotAssess.interval_list")
    add(kb)
  }


  case class assess (@Input inVCF: File, @Input inBam : File) extends AssessNA12878 with UNIVERSAL_GATK_ARGS{
    this.variant :+= inVCF
  ...
  case class filter (@Input inVCF: File, @Output outVCF: File) extends VariantFiltration with UNIVERSAL_GATK_ARGS{
  ...
  case class ug_rnaMode(inBam:File, outVCF: File) extends UnifiedGenotyper with UNIVERSAL_GATK_ARGS{
  ...
  case class hc_rnaMode(inBam:File, outVCF: File) extends HaplotypeCaller with UNIVERSAL_GATK_ARGS{
    this.intervalsString = qscript.intervalString
    this.intervals = qscript.intervalFiles
    this.scatterCount = qscript.jobs
    this.input_file :+= inBam
