...
class Qscript_VariantAnnotator extends QScript {

  @Argument(shortName = "L",  required=false, doc = "Intervals file")
  var intervalsFile: List[File] = Nil
  @Argument(shortName = "normal",  required=true, doc = "Normal sample BAM list")
  var normalBAM: String = ""
  @Argument(shortName = "tumor", required=true, doc = "Tumor sample BAM list")
  var tumorBAM: String = ""
  @Argument(shortName = "pon", required=false, doc = "Panel of normals")
  var panelOfNormals: String = "/dsde/working/mutect/panel_of_normals/panel_of_normals_m2_paad/agilent_hg19_m2_paad_149_normal_panel.vcf"
  @Argument(shortName = "o",  required=true, doc = "Output prefix")
  var outputPrefix: String = ""
  @Argument(shortName = "sc",  required=false, doc = "base scatter count")
  var scatter: Int = 50 
//  @Argument(shortName = "contanFile", required=false, doc = "file specifying contamination fraction")
//  var contan: File = _
  @ClassType(classOf[Double])
  @Argument(shortName = "f", doc = "list of contamination fractions in tumors") var fractionList: List[Double] = _

    def script() {

    val normalBams = QScriptUtils.createSeqFromFile(normalBAM)
    val tumorBams = QScriptUtils.createSeqFromFile(tumorBAM)
    
   for (normalIndex <- 0 until normalBams.size) {
     for (tumorIndex <- 0 until tumorBams.size) {
       //if (normalIndex != tumorIndex)
          add (createM2Config(tumorBams(tumorIndex), normalBams(normalIndex), new File(panelOfNormals), outputPrefix, fractionList(tumorIndex)))
     }
   }
