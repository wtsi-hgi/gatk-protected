 ...
 */
public class AlignmentUtilsBenchmark extends SimpleBenchmark {
    private enum DataSet {
        EMPTY_CIGAR,
        SINGLETON_CIGAR,
        LONG_CIGARS_TO_COMBINE,
        LONG_CIGAR_NO_COMBINE,
        SHORT_CIGAR_NO_COMBINE,
        SHORT_CIGAR_TO_COMBINE
    }

    @Param({"LONG_CIGARS_TO_COMBINE", "EMPTY_CIGAR", "SINGLETON_CIGAR", "LONG_CIGAR_NO_COMBINE", "SHORT_CIGAR_NO_COMBINE"})
    DataSet dataSet;

    @Param({"orig", "fast"})
    String algorithm;

    final int nCigars = 10000;

    final Map<DataSet, List<Cigar>> data = new EnumMap<DataSet, List<Cigar>>(DataSet.class);

    @Override protected void setUp() {
        for ( final DataSet dataSet : DataSet.values() ) {
            final List<Cigar> elts = new LinkedList<Cigar>();
            ...
            data.put(dataSet, elts);
        }
    }

    public void timeConsolidateCigar(int rep) {
        final boolean orig = algorithm.equals("orig");
        for ( int i = 0; i < rep; i++ ) {
            for ( final Cigar cigar : data.get(dataSet) ) {
                if ( orig )
                    originalConsolidateCigar(cigar);
                else
                    AlignmentUtils.consolidateCigar(cigar);
            }
        }
    }

    private static Cigar originalConsolidateCigar( final Cigar c ) {
