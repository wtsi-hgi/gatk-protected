...
public class RFAWalker extends ReadWalker<SAMRecord,RFWindow> {
    ...
    private double calculateGenotypeFreeSkew(Map<String,List<BinaryFeatureAggregator>> window, int offset) {
        Map<String,GenotypeLikelihoods> caseGenotypeLikelihoods = rfaglcm.getLikelihoods(null,null,unwrap(window,offset,true));
        Map<String,GenotypeLikelihoods> controlGenotypeLikelihoos = rfaglcm.getLikelihoods(null,null,unwrap(window,offset,false));
        double[] caseSpectrum = new double[nCase];
        double[] controlSpectrum = new double[nControl];
        ExactAFCalculationModel.rfaN2GoldStandard(caseGenotypeLikelihoods,caseSpectrum.clone(),caseSpectrum,0,1,2);
