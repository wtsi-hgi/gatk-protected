 ...
 */
public class SplitByRG extends ReadWalker<Long, Long> {
    final Map<String, SAMFileWriter> out = new Object2ObjectOpenHashMap<String, SAMFileWriter>();
    ...
    public Long map(ReferenceContext ref, GATKSAMRecord read, RefMetaDataTracker metaDataTracker) {
        final SAMFileWriter writer = out.get(read.getReadGroup().getId());
    ...
    /**
     * Make a new SAMFileHeader with the only read group being included in the file
     *
     * @param rg     the (only) read group to be included
     * @param header original header to copy from
     * @return a new header with rg as the only read group record
     */
    private static SAMFileHeader discardReadGroups(final SAMReadGroupRecord rg, final SAMFileHeader header) {
        final SAMFileHeader newHeader = header.clone();
        final List<SAMReadGroupRecord> rgList = new ObjectArrayList<>(1);
        rgList.add(rg);
