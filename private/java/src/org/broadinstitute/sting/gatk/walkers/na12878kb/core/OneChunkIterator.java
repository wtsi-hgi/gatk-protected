...
public class OneChunkIterator<T extends MongoVariantContext> extends PeekableIterator<T> implements Iterable<T>, CloseableIterator<T>, SiteIterator<T> {
    ...
    /**
     * Create a new OneChunkIterator using parser to make GenomeLocs and reading data from the DBCursor cursor
     ...
     */
    protected OneChunkIterator(GenomeLocParser parser, DBCursor cursor) {
        this(parser, cursor, null, null);
    }
     ...
     * @param parser a GenomeLoc parser creating genome locs, consistent with the data in the cursor
     * @param cursor an initialized DBCursor pointing to data containing values of type T
     * @param intervals the intervals to select from, can be null (in which case use all records)
     * @param errorHandler the error handler
     */
    protected OneChunkIterator(GenomeLocParser parser, DBCursor cursor, final List<GenomeLoc> intervals, final InvalidRecordHandler<T> errorHandler) {
        super(new RawSiteIterator<T>(cursor));

        if ( parser == null ) throw new IllegalArgumentException("Parser cannot be null");
        this.parser = parser;
        this.intervals = intervals == null ? null : GenomeLocSortedSet.createSetFromList(parser, intervals);
        if ( errorHandler != null )
            setErrorHandler(errorHandler);
    }

    /**
     * Provides this OneChunkIterator with an InvalidRecordHandler handler to deal with
    ...
    public List<T> getNextEquivalents() {
        final LinkedList<T> variants = new LinkedList<>();
    ...
    public boolean hasNext() {
        while ( super.hasNext() ) {
            ...
            try {
                n.validate(parser);
                if ( ! occursAtUsableInterval(n.getLocation(parser)) ) {
                    super.next();
                    continue;
                }
                return true;
