...
public class SlidingWindow {
        ...
        if (start < end) {
            ...
            for (HeaderElement wh : windowHeader) {
                if (i == end)
                    break;
                if (i >= start) {
                    if (!wh.hasConsensusData()) {                             // Element may be an empty element representing a gap between the reads
                        GATKSAMRecord consensus = finalizeConsensus();        // we finalize the running consensus
                        if(consensus != null)
                            consensusList.add(consensus);
                        consensusList.addAll(addToSyntheticReads(i + 1, end));     // and continue to add the next section with a recursive call
                        break;
                    }
                    else {
                        byte base  = wh.consensusBaseCounts.baseWithMostCounts();
                        byte count = (byte) Math.min(wh.consensusBaseCounts.countOfMostCommonBase(), MAX_QUAL_COUNT);
                        byte qual  = wh.consensusBaseCounts.averageQualsOfMostCommonBase();
    ...
    /**
     * Looks for the next position with consensus data or empty
     *
     ...
     */
    protected int findNextConsensusElementIndex (int start, int upTo) {
        Iterator<HeaderElement> headerElementIterator = windowHeader.listIterator(start);
        int index = 0;
        while(index < upTo && headerElementIterator.hasNext()) {
            HeaderElement headerElement = headerElementIterator.next();
            if (headerElement.hasConsensusData() || headerElement.isEmpty())
                break;
            index++;
        }
        return index;
     ...
     * @return a list of consensus reads generated by this call. Empty list if no consensus was generated.
     */
    protected void addToFilteredData (int start, int end) {
        if (filteredDataConsensus == null)
            filteredDataConsensus = new SyntheticRead(header, readGroupAttribute, contig, contigIndex, filteredDataReadName + filteredDataConsensusCounter++, windowHeader.get(start).location, MIN_BASE_QUAL_TO_COUNT);
        ...
        for (int index = start; index < end; index++) {
            ...
            byte base  = headerElement.filteredBaseCounts.baseWithMostCounts();
