...
totalcalled["SNPs",   "N_TP"] <- max(subset(roc, variation=="SNPs")$N_TP)
totalcalled["Indels", "N_TP"] <- max(subset(roc, variation=="Indels")$N_TP)
totalcalled["SNPs",   "N_FP"] <- max(subset(roc, variation=="SNPs")$N_FP)
totalcalled["Indels", "N_FP"] <- max(subset(roc, variation=="Indels")$N_FP)
...
ggp_roc_count <- ggplot(data=roc[-c(u_idx),], aes(x=N_FP, y=N_TP)) +
geom_line() +
