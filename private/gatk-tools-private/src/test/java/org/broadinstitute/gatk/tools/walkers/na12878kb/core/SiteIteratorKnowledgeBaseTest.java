...
public class SiteIteratorKnowledgeBaseTest extends NA12878KnowledgeBaseTestBase {
    ...
    private SiteManager makeSiteManager() {
        return new SiteManager(parser, GenomeLocSortedSet.createSetFromSequenceDictionary(fasta.getSequenceDictionary()), fasta.getSequenceDictionary());
    }
    ...
    public void setup() {
        ...
        it = db.getCalls(makeSiteManager());
        it.setErrorHandler(new InvalidRecordsThrowError<>());
    }
    ...
    public void testBeforeIndels(final List<MongoVariantContext> mvcs, final int startThres, final int expectedCount) {
        ...
        it = db.getCalls(makeSiteManager());
        final List<MongoVariantContext> l = it.getSitesBefore(parser.createGenomeLoc("20", startThres, startThres));
        Assert.assertEquals(l.size(), expectedCount, "Query returned different results than expected");
        for ( final MongoVariantContext mvc : l )
            Assert.assertTrue(parser.createGenomeLoc(mvc.getVariantContext()).startsBefore(parser.createGenomeLoc("20", startThres, startThres)), "MVC " + mvc + " has start > threshold " + startThres);
    }

    @DataProvider(name = "BeforeIndelsMultiContig")
    public Object[][] makeBeforeIndelsMultiContig() {
        List<Object[]> tests = new ArrayList<>();

        final MongoVariantContext mvc0_1 = MongoVariantContext.create("x", "1", 1, "A", "C", false); // site before chr20
        final MongoVariantContext mvc0_2 = MongoVariantContext.create("x", "7", 189798, "A", "C", false); // site before chr20
        final MongoVariantContext mvc1 = MongoVariantContext.create("x", "20", 1, "A", "C", false);
        final MongoVariantContext mvc2 = MongoVariantContext.create("x", "20", 2, "ACT", "C", false);
        final MongoVariantContext mvc2_1 = MongoVariantContext.create("x", "20", 2, "ACTGT", "C", false);
        final MongoVariantContext mvc3 = MongoVariantContext.create("x", "20", 3, "A", "G", false);
        final MongoVariantContext mvc4 = MongoVariantContext.create("x", "20", 4, "A", "C", false);
        final MongoVariantContext mvc5 = MongoVariantContext.create("x", "X", 4, "A", "C", false); // site after chr20
        final List<MongoVariantContext> mvcs = Arrays.asList(mvc0_1, mvc0_2, mvc1, mvc2, mvc2_1, mvc3, mvc4, mvc5);

        tests.add(new Object[]{mvcs, 1, 2});
        tests.add(new Object[]{mvcs, 2, 3});
        tests.add(new Object[]{mvcs, 3, 5});
        tests.add(new Object[]{mvcs, 4, 6});

        return tests.toArray(new Object[][]{});
    }

    @Test(enabled = true, dataProvider = "BeforeIndelsMultiContig")
    public void testBeforeIndelsMultiContig(final List<MongoVariantContext> mvcs, final int startThres, final int expectedCount) {
        db.reset();
        db.addCalls(mvcs);
        it = db.getCalls(makeSiteManager());
        final List<MongoVariantContext> l = it.getSitesBefore(parser.createGenomeLoc("20", startThres, startThres));
    ...
    public void testAt(final int start, final int expectedCount) {
        db.addCall(MongoVariantContext.create("x", "1", 4, "A", "C", false));
        db.addCall(MongoVariantContext.create("x", "7", 1, "T", "C", false));
        db.addCall(MongoVariantContext.create("x", "X", 2, "T", "TC", false));
    ...
    public void testBasicIteration(final List<MongoVariantContext> mvcs) {
        ...
        final SiteIterator<MongoVariantContext> it = db.getCalls(makeSiteManager());
    ...
    public void testIteratorWithBadMVCsLogError(final MongoVariantContext bad) {
        ...
        db.addCall(bad);
        it = db.getCalls();
        final InvalidRecordsLogError<MongoVariantContext> handler = new InvalidRecordsLogError<>();
    ...
    public void testIteratorWithBadMVCsRemoving(final MongoVariantContext bad) {
        ...
        db.addCall(bad);
        it = db.getCalls();
        final InvalidRecordsRemove<MongoVariantContext> handler = new InvalidRecordsRemove<>(db.sites);
        it.setErrorHandler(handler);
        final List<MongoVariantContext> withoutBad = it.toList();
        Assert.assertEquals(withoutBad, expected);
        Assert.assertEquals(handler.getnBad(), 1);
        ...
        it = db.getCalls();
        final InvalidRecordsThrowError<MongoVariantContext> errorThrower = new InvalidRecordsThrowError<>();
