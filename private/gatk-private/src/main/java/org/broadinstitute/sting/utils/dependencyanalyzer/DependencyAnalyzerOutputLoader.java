...
/**
 * Allows output from the ant-based walker dependency analyzer tool (private/gsa-engineering/walker_dependency_analyer)
 * to be loaded and queried. Once loaded, you can determine the analysis result for either an individual class or
 * a list of classes.
 *
 * @author David Roazen
 */
public class DependencyAnalyzerOutputLoader {
    ...
    /**
     * Loads the output from a run of the walker dependency analyzer tool (private/gsa-engineering/walker_dependency_analyer)
     * so that the individual results for each walker can be queried.
     *
     * @param dependencyAnalyzerOutputFileName String containing path to a valid output file produced by the walker
     *                                         dependency analyzer
     */
    public DependencyAnalyzerOutputLoader( final String dependencyAnalyzerOutputFileName ) {
        try {
            FileInputStream inputStream = null;
            try {
                inputStream = new FileInputStream(new File(dependencyAnalyzerOutputFileName));
                dependencyAnalyzerResults.load(inputStream);
            }
            finally {
                if ( inputStream != null ) {
                    inputStream.close();
            ...
            throw new RuntimeException(String.format("Error loading dependency analyzer output file %s. Error was: %s",
                                                     dependencyAnalyzerOutputFileName, e.getMessage()));
        }
    }
     ...
     */
    public boolean classHasDependencies( final String className ) {
        final String classResult = dependencyAnalyzerResults.getProperty(className);
        if ( classResult == null ) {
     ...
     */
    public List<Boolean> classesHaveDependencies( final List<String> classNames ) {
        final List<Boolean> classResults = new ArrayList<Boolean>(classNames.size());

        for ( final String className : classNames ) {
            classResults.add(classHasDependencies(className));
        }
