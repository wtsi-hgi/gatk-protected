...
import org.broadinstitute.sting.gatk.walkers.haplotypecaller.HeterogeneousKmerSizeResolution;

class GraphHMMEvaluation extends QScript {
  qscript =>

  // Required arguments.  All initialized to empty values.

  @Argument(shortName = "kmerSize", doc = "Kmer sequences", required = false)
  @ClassType(classOf[Int])
  var kmerSize : Seq[Int] = Nil 
 
  @Argument(shortName = "useVQSR", doc = "Request to run VQSR", required = false)
  var recalVariants : Boolean = false
  ...
  @Argument(shortName = "hksr", doc = "xxx", required = false)
  var heterogeneousKmerSizeResolution : HeterogeneousKmerSizeResolution = HeterogeneousKmerSizeResolution.COMBO_MAX

  

  @Argument(shortName = "R", doc = "Obvious", required = true)
  ...
  trait BaseCommandArguments extends CommandLineGATK with RetryMemoryLimit {
    ...
    if (excludeBadRegions)
      this.excludeIntervalsString = Seq("20:25,730,140-26,319,926", "20:29,419,342-29,655,147")
  }
  ...
  class BQSR(bam: File, recal: File) extends BaseRecalibrator with BaseCommandArguments {
    ...
    this.scatterCount = 20 //if(qscript.scatterCount == 0) 200 else qscript.scatterCount
  }
  ...
  class MyHaplotypeCaller(name: String, inputHCFile: File) extends HaplotypeCaller with BaseCommandArguments {
    ...
    //this.bamOutput = qscript.outputDir + "/" + name + ".HaplotypeCaller.bam"
    this.analysisName = "HaplotypeCaller"
    this.javaGCThreads = 4
    this.memoryLimit = 8
    this.heterogeneousKmerSizeResolution = qscript.heterogeneousKmerSizeResolution
    this.kmerSize = qscript.kmerSize
    this.dontTrimActiveRegions = true
    this.jobName = queueLogDir + "IllumminaBinning.hc"
  }

  class LoglessHaplotypeCaller(name: String, inputHCFile: File) extends MyHaplotypeCaller(name + ".Ll",inputHCFile) {
    this.jobName += ".ll"
  }

  class GraphHaplotypeCaller(name: String, inputHCFile: File) extends MyHaplotypeCaller(name + ".Gh",inputHCFile) {
    this.jobName += ".gh"
    this.fastAndFurious = true;
  }
  ...
  def liftOverChainFile: File = {
    new File(qscript.chainFileDir, qscript.inBuildName + "to" + qscript.outputBuildName + ".chain")
  }

  def liftOver(in: File): File = {
    val out = outBuildFileName(in);
    if (in == out) in
    else {
      val myLiftOver = new MyLiftOver(in, out, liftOverChainFile)
      add(myLiftOver)
      myLiftOver.out
    }
  }
  ...
  class MyAssessAgainstKB(overrideIntervals: String = null) extends AssessNA12878 with BaseCommandArguments {
    ...
    def ext() = if (overrideIntervals != null) overrideIntervals + ".txt" else ".txt"
  }
  ...
  def script() {
    ...
    for ((calibratedBam, reducedBam) <- bams) {
      val reduce = new ReduceReads with BadMate with RetryMemoryLimit with BaseCommandArguments
    ...
    for (pipelineVersion <- pipelineVersions) {
      ...
      val finalCallset: File = if (pipelineVersion % 2 == 1) {
          val genotyper = new LoglessHaplotypeCaller(baseName, genotypingBam)
          add(genotyper)
          val finalCallset = if (recalVariants) recalibrateSNPsAndIndels(liftOver(genotyper.out), true) else genotyper.out
          callSets += (("Ll." + genotypingBam.getName) -> finalCallset)
          finalCallset
        } else {
          val genotyper = new GraphHaplotypeCaller(baseName, genotypingBam)
          add(genotyper)
          val finalCallset = if (recalVariants) recalibrateSNPsAndIndels(genotyper.out, false) else genotyper.out
          callSets += (("Gh." + genotypingBam.getName) -> finalCallset)
          finalCallset
        }
      ...
      for (assessKB <- List(new MyAssessAgainstKB(), new MyAssessAgainstKB("20:10000000-24000000"))) {
