...
class RealignAroundIndels extends QScript {
  ...
  val bams: List[File] = List()

  @Argument(shortName = "vcf", doc="vcfs", required=true)
  val vcfs: List[File] = List()
  ...
  def script() {
    ...
    for ( vcf <- vcfs ) {
      ...
      sv.V = vcf
      sv.selectTypeToInclude = List(VariantContext.Type.INDEL)
      sv.o = swapExt(outdir, vcf, ".vcf", "_indels.vcf")
      add(sv)

      val rtc = new RealignerTargetCreator with UNIVERSAL_GATK_ARGS
      rtc.known :+= sv.o
      rtc.o = swapExt(outdir, sv.o, ".vcf", ".intervals")
      add(rtc)

      for ( bam <- bams ) {
        val ir = new IndelRealigner with UNIVERSAL_GATK_ARGS
        ir.targetIntervals = rtc.o
        ir.I :+= bam
        ir.known :+= sv.o
        ir.LOD = Option(1)
        ir.consensusDeterminationModel = ConsensusDeterminationModel.KNOWNS_ONLY
        ir.out = swapExt(outdir, vcf, ".vcf", "_" + bam.getName)
        add(ir)
      }
    }
  }
