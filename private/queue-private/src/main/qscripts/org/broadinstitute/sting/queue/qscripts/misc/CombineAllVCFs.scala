...
class CombineAllVCFs extends QScript {
  @Input(shortName = "vcfSrcDir", doc="The source directory of VCFs to process", required=false)
  val vcfSrcDir: File = "/home/unix/thibault/scratch/vs/s_size"

  @Output(shortName = "out", doc="The output VCF", required=false)
  val out: File = "/home/unix/thibault/scratch/vs/all_size.vcf"
  ...
  def script() {
    ...
    cv.variant = new File(vcfSrcDir).listFiles.filter(_.getName.endsWith(".vcf"))
    cv.out = out
