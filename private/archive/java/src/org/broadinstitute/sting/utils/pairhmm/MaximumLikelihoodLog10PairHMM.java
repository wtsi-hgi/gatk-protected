 ...
 */
public class MaximumLikelihoodLog10PairHMM extends Log10PairHMM {
    /**
     * Create an uninitialized PairHMM
     *
     * @param doExactLog10 should the log10 calculations be exact (slow) or approximate (faster)
     */
    public MaximumLikelihoodLog10PairHMM(final boolean doExactLog10) {
        super(doExactLog10);
    ...
    protected void updateCell(final int indI, final int indJ, final double prior, final double[] transition) {
        if (!graphLikelihoodTest) {
            super.updateCell(indI, indJ, prior, transition);
            return;
        }
    ...
    protected double finalLikelihoodCalculation() {
        ...
        double finalSumProbabilities = Math.max(matchMatrix[endI][1], insertionMatrix[endI][1]);
        for (int j = 2; j < paddedHaplotypeLength; j++)
            finalSumProbabilities = Math.max(finalSumProbabilities, Math.max(matchMatrix[endI][j], insertionMatrix[endI][j]));
